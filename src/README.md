...
5. `calculate_score.py` — scoring. Excludes parcels with any null soil
...
6. `subzone_summary.py` — scoring. Joins `parcel_scores` with
  ...
  this fixed.
7. `cross_project_comparison.py` — scoring. Joins `subzone_summary`
...
8. `ingest_climate_risk.py` — production ingestion. Computes one
  ...
  Whakatane was added.
9. `regional_summary_expansion.py` — scoring. Two parts: (1) groups all
